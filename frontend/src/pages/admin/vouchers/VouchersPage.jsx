...
const VouchersPage = () => {
  ...
  const handleStatusChange = async (voucherId, newStatus) => {
    try {
      ...
      // Refresh the vouchers list to get updated computed status from backend
      const response = await voucherAPI.getVouchers({
        page: currentPage,
        limit: vouchersPerPage,
        search: searchTerm,
  ...
  const handleDeleteVoucher = async (voucherId) => {
    if (window.confirm('Bạn có chắc chắn muốn xóa voucher này?')) {
      try {
        ...
        // Refresh the vouchers list after deletion
        const response = await voucherAPI.getVouchers({
          page: currentPage,
          limit: vouchersPerPage,
          search: searchTerm,
                ...
                <tr key={voucher._id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <select
                      ...
                      className={`text-xs font-medium px-3 py-1 rounded-full ${getStatusColor(voucher.status)} border-0 focus:ring-2 focus:ring-blue-500`}
